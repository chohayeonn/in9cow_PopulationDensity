...
Android 앱과 Spring Boot 서버 간 연동 구현 및 전체 시스템 구조 설계

Retrofit을 활용한 REST API 통신 구현, 예측·추천·즐겨찾기 기능 연동

데이터베이스(DB: AWS RDS MySQL)와 앱/서버 간 연동 설계 및 테스트

주요 성과: 실시간 혼잡도 지도 시각화, AI 예측 결과 조회, 추천 및 즐겨찾기 기능이 정상 동작하도록 전체 앱 기능 안정화
